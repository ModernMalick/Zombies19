...
module Utilities =
        ...
        | PersonNode(person, children) when person = origin ->
            ...
            let updatedChildren = 
                ...
                |> List.mapi (fun index child -> 
                    ...
                    | PersonNode(childPerson, childChildren) ->
                        if index % 2 = 0 then
                            PersonNode({ childPerson with Status = Status.ZombieC }, childChildren)
                        else 
                            child
                    | Empty -> Empty)
            PersonNode(updatedPerson, updatedChildren)
        | PersonNode(person, children) -> PersonNode(person, List.map (fun child -> infectZombieC child origin) children)
        | Empty -> Empty
    ...
    let rec vaccinateZombieA1 tree =
        match tree with
        | PersonNode(person, children) when person.Age <= 30 && (person.Status = Status.ZombieA || person.Status = Status.Zombie32) ->
    ...
    let rec vaccinateZombieB1 tree =
        let rec vaccinateHelper children index =
            match children with
            | [] -> []
            | PersonNode(person, childChildren) :: rest when person.Status = Status.ZombieB || person.Status = Status.ZombieC ->
